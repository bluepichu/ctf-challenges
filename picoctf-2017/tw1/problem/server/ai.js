...
module.exports = function(state, entity){
	...
	if (curDist <= 3 || (playerRoom > 0 && playerRoom == entityRoom)) {
		...
		for (var i = 0; i < 8; i++) {
			if (validMove(state, entity, { action: "move", direction: i })) {
	...
	if (state.map.grid[entity.location.r][entity.location.c] == 0) {
		...
		for (var i = 0; i < offsets.length; i++) {
			...
			if (validMove(state, entity, { action: "move", direction: movedir })) {
	...
	return { action: "move", direction: entity.lastMoveDir };
}
