...
module.exports = function(app, io){
	io.on("connection", function(socket){
		...
		var rnd = Math.random() * 4294967296;

		socket.name = name(rnd);

		var r = ((rnd >> 26) & 0x3f) + 0xc0;
		var g = ((rnd >> 19) & 0x7f) + 0x80;
		var b = ((rnd >> 13) & 0x3f) + 0xc0;
		...
		socket.on("disconnect", function(){
			...
			if(socket.id in db.scoreboard){
				delete db.scoreboard[socket.id];
			}
		...
		socket.on("subscribe", function(){
			// logger.info("[scoreboard]", socket.id, "has subscribed");
			socket.join("scoreboard");
			socket.emit("scoreboard", db.censorScoreboard(io));
		});
	...
	setInterval(function(){
		// logger.info("[scoreboard] Scoreboard Update");
		io.to("scoreboard").emit("scoreboard", db.censorScoreboard(io));
	}, 5000);
}
